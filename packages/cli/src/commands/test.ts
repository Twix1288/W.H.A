...
export async function testAgent(scriptPath: string) {
	...
	const _vmScriptPath = await provider.copyIn(absolutePath);
	...
	socketClient.on("network_block", (event) => {
		console.log(
			`  🚨 [BLOCKED] eBPF intercepted unauthorized connection to ${event.dst_ip}:${event.dst_port}`,
		);
	});

	socketClient.on("syscall_violation", (event) => {
		console.log(
			`  🚨 [FATAL] Sandbox intercepted forbidden syscall: ${event.syscall}`,
		);
	});

	socketClient.on("prompt_injection", (event) => {
		console.log(
			`  ⚠️ [WARNING] Middleware detected Prompt Injection attempt with ${(event.confidence * 100).toFixed(0)}% confidence.`,
		);
	});
	...
	try {
		...
		console.log(
			`👉 View full forensic timeline: shield logs --session=${Date.now()}`,
		);
	}
}
