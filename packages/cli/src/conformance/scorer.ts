	...
	async score(artifact: Buffer): Promise<ConformanceScore> {
				...
				diagnostics: [
					{
						...
						message: `Failed to extract SKILL.md: ${e.message}`,
					},
				],
				passed: false,
			};
		...
		if (
			...
			diagnostics.push({
				line: 1,
				severity: "warning",
				ruleId: "structure_missing_examples",
				message: "Package is missing usage examples in its markdown",
			});
		}
		...
		if (
			...
			diagnostics.push({
				line: 1,
				severity: "warning",
				ruleId: "security_missing_section",
				message: "No explicit security or permission constraints mentioned",
			});
		}

		const totalScore =
			Math.max(0, frontmatterScore) +
