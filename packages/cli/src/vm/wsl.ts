...
export class WslProvider implements VMProvider {
    ...
    public async isAvailable(): Promise<boolean> {
        try {
            const { stdout, stderr } = await execAsync('wsl --status');
            return true;
        } catch (e: any) {
            if (e.message.includes('0x80070005')) {
                console.warn("[WARNING] WSL2 is blocked by Corporate Group Policy (0x80070005). Falling back to Docker Desktop.");
                return false;
            }
            console.error("\n[ERROR] WSL2 is not enabled on this Windows machine.");
            console.error("W.H.Agent requires WSL2 to provide Tier A Firecracker isolation.");
            console.error("Please run: 'wsl --install' from an Administrator PowerShell and reboot.\n");
            return false;
        }
    }
    ...
    public async copyIn(localPath: string): Promise<string> {
        ...
        await execAsync(`powershell.exe -Command "Get-Content -Path '${winPath}' -Raw | wsl -d ${this.distroName} cat > ${dest}"`);
