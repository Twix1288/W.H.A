...
export class LimaProvider implements VMProvider {
    ...
    public async setup(): Promise<void> {
        ...
        const { stdout: kernel } = await execAsync(`limactl shell ${this.instanceName} uname -r`);
        if (kernel.trim() !== this.expectedKernel) {
            ...
            console.error(`eBPF probes will silently fail on unpinned kernels. Please run 'shield setup' to rebuild the VM.\n`);
            process.exit(1);
        }
    }
