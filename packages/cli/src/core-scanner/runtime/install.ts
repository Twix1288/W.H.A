...
export const RUNTIME_HOOK_MARKER = "wh-agent/runtime-policy";

const HOOK_COMMAND =
	"node -e \"const fs=require('fs'),p=require('path');const s=Date.now();const t=process.env.TOOL_NAME||'unknown';const i=process.env.TOOL_INPUT||'';const pp=p.resolve('.wh-agent/runtime-policy.json');if(!fs.existsSync(pp)){process.exit(0)}const pol=JSON.parse(fs.readFileSync(pp,'utf-8'));for(const r of pol.deny||[]){if(r.tool==='*'||r.tool===t||t.startsWith(r.tool.replace('*',''))){if(!r.pattern||new RegExp(r.pattern,'i').test(i)){const lp=p.resolve((pol.log||{}).path||'.wh-agent/runtime.ndjson');const d=p.dirname(lp);if(!fs.existsSync(d))fs.mkdirSync(d,{recursive:true});fs.appendFileSync(lp,JSON.stringify({timestamp:new Date().toISOString(),tool:t,decision:'block',reason:r.reason,durationMs:Date.now()-s})+'\\n');process.stderr.write('W.H.Agent: BLOCKED '+t+' — '+(r.reason||'denied by policy')+'\\n');process.exit(2)}}}const lp2=p.resolve((pol.log||{}).path||'.wh-agent/runtime.ndjson');const d2=p.dirname(lp2);if(!fs.existsSync(d2))fs.mkdirSync(d2,{recursive:true});fs.appendFileSync(lp2,JSON.stringify({timestamp:new Date().toISOString(),tool:t,decision:'allow',durationMs:Date.now()-s})+'\\n');process.exit(0)\"";
...
function runtimePolicyPath(targetPath: string): string {
	return join(targetPath, ".wh-agent", "runtime-policy.json");
}
...
function repairHint(): string {
	return "Run `wh-agent runtime repair` to back up invalid files and restore a healthy runtime monitor.";
}

function nextBackupPath(filePath: string): string {
	const basePath = `${filePath}.wh-agent.bak`;
...
function installRuntimeAtPath(
	...
	if (alreadyInstalled) {
		return {
			...
			settingsPath,
			policyPath,
			message: "W.H.Agent runtime hook is already installed.",
		};
	}
	...
	return {
		...
		settingsPath,
		policyPath,
		message: "W.H.Agent runtime hook installed successfully.",
	};
}

/**
 * Install the W.H.Agent runtime hook into settings.json
...
/**
 * Repair the W.H.Agent runtime monitor by backing up unreadable config
...
export function repairRuntime(targetPath: string): RuntimeRepairResult {
	...
	const changed = Boolean(
		settingsBackupPath ||
			policyBackupPath ||
			installResult.hookInstalled ||
			installResult.policyCreated,
	);
	...
	return {
		...
		message: repaired
			? changed
				? "W.H.Agent runtime monitor repaired successfully."
				: "W.H.Agent runtime monitor is already healthy."
			: `W.H.Agent runtime monitor is still not ready. ${status.message}`,
	};
}

/**
 * Uninstall the W.H.Agent runtime hook from settings.json.
 */
export function uninstallRuntime(targetPath: string): {
	...
	if (!settingsState.valid) {
		return {
			removed: false,
			message: `Failed to parse settings.json. ${repairHint()}`,
		};
	}
	...
	const filtered = preToolUse.filter(
		(h) =>
			!(typeof h.hook === "string" && h.hook.includes(RUNTIME_HOOK_MARKER)),
	);

	if (filtered.length === preToolUse.length) {
		return { removed: false, message: "W.H.Agent runtime hook not found." };
	}
	...
	return { removed: true, message: "W.H.Agent runtime hook removed." };
}
