...
function annotateWarning(
	...
	console.log(
		`::warning file=${file}${lineParam}::${escapeAnnotation(message)}`,
	);
}
...
async function run(): Promise<void> {
	...
	const policyPromotionOutput = getInput(
		"policy-promotion-output",
		".wh-agent/policy.json",
	);
	const policyPromotionDryRun =
		getInput("policy-promotion-dry-run", "true") === "true";
	const failOnPolicyPromotion =
		getInput("fail-on-policy-promotion", "false") === "true";
	...
	const verifyEvidencePackOutput =
		getInput("verify-evidence-pack", "true") === "true";
	...
	const filteredResult = {
		...
		findings: result.findings.filter((f) =>
			isAtOrAboveSeverity(f, minSeverity),
		),
	};

	// Calculate score
	const report = calculateScore(filteredResult);
	const packageManagerHardening = summarizePackageManagerHardening(
		result.findings,
	);
	...
	if (policyPromotionManifest) {
		try {
			const { PolicyPackSchema, promotePolicyPack } = await import(
				"./policy/index.js"
			);
			...
			console.log(
				`::error::W.H.Agent policy promotion failed: ${escapeAnnotation(message)}`,
			);
			process.exitCode = 1;
			return;
		}
	...
	if (policyPath) {
		...
		if (!policyResult.success) {
			...
			setOutput(
				"policy-violations",
				String(policyEvaluation.violations.length),
			);
	...
	if (policyPromotionResult) {
		...
		setOutput(
			...
			String(promotionSummary.totalReviewItems),
		);
		setOutput(
	...
	writeJobSummary(
		renderPackageManagerHardeningJobSummary(packageManagerHardening),
	);
	...
	if (supplyChainRequested || supplyChainOnline || evidencePackPath) {
		try {
			const { extractPackages, renderSupplyChainReport, verifyPackages } =
			...
			setOutput(
				...
				String(supplyChainReport.riskyPackages),
			);
			setOutput(
				"supply-chain-critical-count",
				String(supplyChainReport.criticalCount),
			);
			setOutput("supply-chain-high-count", String(supplyChainReport.highCount));
			writeJobSummary(
				renderSupplyChainJobSummary(supplyChainReport, {
					online: supplyChainOnline,
					failOnSupplyChain,
				}),
			);
			...
			if (
				...
				console.log(
					`::error::W.H.Agent supply-chain gate FAILED: ${reason} package risk(s)`,
				);
			...
			console.log(
				`::error::W.H.Agent supply-chain verification failed: ${escapeAnnotation(message)}`,
			);
			process.exitCode = 1;
			return;
		}
	...
	if (baselinePath) {
		const { loadBaseline, compareBaseline, evaluateGate } = await import(
			"./baseline/index.js"
		);
		...
		if (baseline) {
			...
			setOutput(
				"resolved-findings",
				String(comparison.resolvedFindings.length),
			);
			...
			if (comparison.newFindings.length > 0) {
				console.log("");
				console.log(
					`Baseline comparison: ${comparison.newFindings.length} new, ${comparison.resolvedFindings.length} resolved`,
				);
			...
			if (!gateResult.passed) {
				console.log("");
				console.log(
					`::error::W.H.Agent gate FAILED: ${gateResult.reasons.join("; ")}`,
				);
			...
			console.log(
				`::warning::Could not load baseline from ${baselinePath}. Skipping comparison.`,
			);
		}
	}
	...
	if (evidencePackPath) {
		try {
			...
			if (verifyEvidencePackOutput) {
				...
				if (!verification.ok) {
					console.log(
						`::error::W.H.Agent evidence pack verification failed: ${escapeAnnotation(verification.errors.join("; "))}`,
					);
					process.exitCode = 1;
					return;
				}
				console.log(
					`Evidence pack verification: PASSED (${verification.bundleDigest})`,
				);
			}
		} catch (error) {
			const message = error instanceof Error ? error.message : String(error);
			...
			console.log(
				`::error::W.H.Agent evidence pack failed: ${escapeAnnotation(message)}`,
			);
			process.exitCode = 1;
			return;
		}
