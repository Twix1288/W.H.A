...
policyCmd
  ...
  .action(async (options) => {
    ...
    if (manifest.packs[0]) {
      console.log(`  Then run: wh-agent scan --policy ${join(options.outputDir, manifest.packs[0].file)}\n`);
    } else {
      console.log("");
    }
  });
...
policyCmd
  ...
  .option("-m, --manifest <path>", "Policy export manifest path", ".wh-agent/policies/manifest.json")
  .option("-o, --output <path>", "Active policy output path", ".wh-agent/policy.json")
  ...
  .action((options) => {
    ...
    server.on("error", (err: NodeJS.ErrnoException) => {
      if (err.code === "EADDRINUSE") {
        ...
        console.error(`  Try a different port: wh-agent miniclaw start --port 4000\n`);
      } else {
