...
export function compareBaseline(
	...
	const resolvedFindings = baseline.findings.filter((f) =>
		baselineFingerprintsFor(f).every(
...
export function evaluateGate(
	...
	if (config.failOnNewHigh && comparison.newHighCount > 0) {
		reasons.push(`${comparison.newHighCount} new high finding(s) introduced`);
	}
...
export function renderComparison(comparison: BaselineComparison): string {
	...
	lines.push(`  Baseline from: ${comparison.baselineTimestamp}`);
	lines.push("");
