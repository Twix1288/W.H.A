import type { PackageVerification, SupplyChainReport } from "./types.js";

// biome-ignore lint/suspicious/noControlCharactersInRegex: intentionally strips terminal control bytes before rendering report text
	...
	if (clean.length > 0) {
		...
		for (const pkg of clean) {
			const version = pkg.package.version
				? `@${escapeControlChars(pkg.package.version)}`
				: "";
...
function renderPackage(
	...
	lines.push(
		`    [${sev}] ${name}${version} (server: ${serverName}, via: ${source})`,
	);

	for (const risk of verification.risks) {
		lines.push(
			`      - [${risk.severity.toUpperCase()}] ${escapeControlChars(risk.description)}`,
		);
		if (risk.evidence) {
