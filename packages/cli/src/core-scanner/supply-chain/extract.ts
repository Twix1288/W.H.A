...
function extractFromConfigFile(
	file: ConfigFile,
): ReadonlyArray<ExtractedPackage> {
	...
	try {
		...
		for (const field of [
			...
			for (const [name, spec] of Object.entries(dependencies)) {
				if (!looksLikePackageDependency(name) || typeof spec !== "string")
					continue;
				packages.push({
					name,
...
function extractFromServerConfig(
	...
	if (command === "node" || command.endsWith("/node")) {
		for (const arg of args) {
			...
			const nodeModuleMatch = arg.match(/node_modules\/(@[^/]+\/[^/]+|[^/]+)/);
			if (nodeModuleMatch) {
				packages.push({
