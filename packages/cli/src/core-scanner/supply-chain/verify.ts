...
export async function verifyPackages(
		...
		const overallSeverity =
			risks.length > 0
				? risks.reduce((worst, r) =>
						SEVERITY_ORDER[r.severity] < SEVERITY_ORDER[worst.severity]
							? r
							: worst,
					).severity
				: "info";
	...
	return {
		...
		criticalCount: riskyPackages.filter((v) => v.overallSeverity === "critical")
			.length,
...
function summarizePackageProvenance(
	...
	return verifications.reduce<SupplyChainProvenanceSummary>(
		(summary, verification) => ({
			npmPackages:
				summary.npmPackages +
				(verification.provenance.ecosystem === "npm" ? 1 : 0),
			gitPackages:
				summary.gitPackages +
				(verification.provenance.ecosystem === "git" ? 1 : 0),
			pinnedPackages:
				summary.pinnedPackages + (verification.provenance.pinned ? 1 : 0),
			unpinnedPackages:
				summary.unpinnedPackages + (verification.provenance.pinned ? 0 : 1),
			knownGoodPackages:
				summary.knownGoodPackages + (verification.provenance.knownGood ? 1 : 0),
			registryMetadataPackages:
				summary.registryMetadataPackages +
				(verification.provenance.metadataSource === "npm-registry" ? 1 : 0),
		}),
		{
			npmPackages: 0,
			gitPackages: 0,
...
async function fetchRegistryMeta(
	...
	try {
		...
		if (latestVersion && versions?.[latestVersion]) {
			const scripts = versions[latestVersion].scripts as
