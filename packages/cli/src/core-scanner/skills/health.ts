...
export function analyzeSkillHealth(
	...
	return {
		...
		rollbackReadySkills: skills.filter((skill) => skill.hasRollbackMetadata)
			.length,
...
export function getSkillProfiles(
	...
	return skillFiles.map((file) => {
		...
		const records = historyFiles.flatMap((historyFile) =>
			parseHistoryFile(historyFile),
		);
...
function hasObservationHooks(frontmatter: SkillFrontmatter): boolean {
	return (
		hasKey(frontmatter, /(?:^|_)(?:observe|observation)(?:_hook|_hooks)?$/) ||
...
function hasFeedbackHooks(frontmatter: SkillFrontmatter): boolean {
	return (
		hasKey(frontmatter, /(?:^|_)feedback(?:_hook|_hooks)?$/) ||
...
function hasRollbackMetadata(frontmatter: SkillFrontmatter): boolean {
	return (
		hasKey(frontmatter, /rollback(?:_strategy|_plan|_metadata)?$/) ||
...
function getRelatedHistoryFiles(
	...
	return files.filter((file) => {
		...
		return (
			HISTORY_SUFFIXES.some((suffix) => lowerName.endsWith(suffix)) &&
...
function extractSuccess(record: Record<string, unknown>): boolean | undefined {
	...
	if (
		["success", "succeeded", "ok", "passed", "completed"].includes(normalized)
	) {
		return true;
	}
	if (
		["failure", "failed", "error", "errored", "rollback", "reverted"].includes(
			normalized,
		)
	) {
		return false;
...
function classifySkillStatus(score: number | undefined): SkillHealth["status"] {
