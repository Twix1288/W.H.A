...
export function inspectEvidencePack(outputDir: string): EvidencePackInspectionResult {
  ...
  const report = readJsonFile<SecurityReport>(resolvedOutputDir, "wh-agent-report.json", errors);
...
function buildFleetReviewReversibleAction(route: EvidencePackFleetRoute): string {
  if (route === "invalid") return "Discard the broken pack and rerun `wh-agent scan --evidence-pack <dir>` from a clean workspace.";
