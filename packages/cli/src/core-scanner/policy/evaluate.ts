...
import type { ConfigFile, Finding, SecurityScore, Severity } from "../types.js";
...
function buildExpiredExceptionViolations(
	...
	return exceptions
		...
		.map((exception) => ({
			...
			expected:
				"Exception must have a future expires_at timestamp or be removed",
...
function buildExceptionSummary(
	...
	return {
		...
		expiringSoon: entries.filter((entry) => entry.status === "expiring_soon")
			.length,
...
function statusForExceptionDays(
...
function exceptionMatchesViolation(
	...
	const haystack = [violation.description, violation.expected, violation.actual]
		.join("\n")
		.toLowerCase();
...
export function renderPolicyEvaluation(evaluation: PolicyEvaluation): string {
	...
	if (evaluation.passed) {
		...
		lines.push(
			`  Status: ${hasExceptions ? "COMPLIANT (WITH EXCEPTIONS)" : "COMPLIANT"}`,
		);
	} else {
	...
	if (evaluation.violations.length > 0) {
		...
		for (const v of evaluation.violations) {
			lines.push(
				`    [${v.severity.toUpperCase().padEnd(8)}] ${v.rule}: ${v.description}`,
			);
	...
	if (evaluation.exceptionSummary && evaluation.exceptionSummary.total > 0) {
		...
		for (const exception of summary.entries) {
			...
			lines.push(
				`    ${exception.id} (${exception.rule}) ${details.join(" ")}`,
			);
		}
		lines.push("");
	}
