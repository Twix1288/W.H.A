...
const HOOK_SHELL_EXTENSIONS = new Set([".sh", ".bash", ".zsh"]);
...
function isExampleOnlyClaudeRoot(
	...
	const hasRuntimeCompanion =
		["settings.json", "settings.local.json", "mcp.json", ".claude.json"].some(
...
function extractCommandPathCandidates(command: string): ReadonlyArray<string> {
	const pathPattern =
		/(?:(?:\$\{[A-Za-z_][A-Za-z0-9_]*\}|\$[A-Za-z_][A-Za-z0-9_]*)\/)?(?:\.{1,2}\/)?(?:[A-Za-z0-9_.-]+\/)*[A-Za-z0-9_.-]+\.(?:sh|bash|zsh|js|cjs|mjs|ts|cts|mts|py|rb)/gi;
