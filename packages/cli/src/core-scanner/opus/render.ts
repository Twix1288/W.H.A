...
export function renderOpusAnalysis(analysis: OpusAnalysis): string {
	...
	if (analysis.attacker.findings.length > 8) {
		lines.push(
			chalk.dim(`    ... and ${analysis.attacker.findings.length - 8} more`),
		);
	}
	lines.push("");
	...
	if (analysis.defender.findings.length > 8) {
		lines.push(
			chalk.dim(`    ... and ${analysis.defender.findings.length - 8} more`),
		);
	}
	lines.push("");
	...
	lines.push(
		`  Risk Level: ${riskColor(analysis.auditor.riskLevel.toUpperCase())}`,
	);
	...
	lines.push(
		chalk.dim(
			"  Powered by Claude Opus 4.6 — three-agent adversarial analysis",
		),
	);
	lines.push("");
