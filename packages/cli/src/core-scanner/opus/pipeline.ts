...
function renderPhaseBanner(
	...
	process.stdout.write(
		colorFn(
			`  \u2503  ${phaseNumber}: ${title.padEnd(divider.length - phaseNumber.length - 4)}\u2503\n`,
		),
	);
	process.stdout.write(
		colorFn(`  \u2503  ${subtitle.padEnd(divider.length - 2)}\u2503\n`),
	);
	process.stdout.write(colorFn(`  \u2517${divider}\u251B\n`));
	process.stdout.write("\n");
}
...
function createSpinner(
	...
	const intervalId = setInterval(() => {
		...
		process.stdout.write(
			`\r  ${spinner} ${label} \u2014 ${chalk.dim(`${lastTokenCount} tokens`)}`,
		);
	}, 80);
 ...
 * Accepts Anthropic SDK ContentBlock[] or any typed-block array.
 */
export function extractTextContent(contentBlocks: ReadonlyArray<any>): string {
	return contentBlocks
...
export function parseAttackerToolCalls(
	...
	const attacks: AttackVector[] = toolCalls
		...
		.map((tc) => ({
			...
			difficulty: String(
				tc.input.difficulty ?? "moderate",
			) as AttackVector["difficulty"],
			cvss_estimate: Number(tc.input.cvss_estimate ?? 5),
			evidence: String(tc.input.evidence ?? ""),
			prerequisites: tc.input.prerequisites
				? String(tc.input.prerequisites)
				: undefined,
		}));
	...
	const goodPractices: GoodPractice[] = toolCalls
		...
		.map((tc) => ({
			...
			effectiveness: String(
				tc.input.effectiveness ?? "moderate",
			) as GoodPractice["effectiveness"],
		}));
...
export function parseAuditorToolCalls(
	...
	const assessmentCall = toolCalls.find(
		(tc) => tc.toolName === "final_assessment",
	);
 ...
 */
function toAttackerPerspective(
	result: StructuredAttackerResult,
): OpusPerspective {
	const findings: string[] = result.attacks.map(
		(a) =>
	...
	return {
		role: "attacker",
		findings:
			findings.length > 0 ? findings : [result.reasoning.substring(0, 500)],
 ...
 */
function toDefenderPerspective(
	result: StructuredDefenderResult,
): OpusPerspective {
...
function toAudit(result: StructuredAuditorResult): OpusAudit {
	...
	return {
		...
		recommendations:
			allRecs.length > 0 ? allRecs : ["Review the full audit output above"],
...
function summarizeAttacker(result: StructuredAttackerResult): string {
	...
	for (const attack of result.attacks) {
		...
		lines.push(
			`- **Impact**: ${attack.impact} | **Difficulty**: ${attack.difficulty} | **CVSS**: ${attack.cvss_estimate}`,
		);
...
function summarizeDefender(result: StructuredDefenderResult): string {
	...
	if (result.gaps.length > 0) {
		...
		for (const gap of result.gaps) {
			lines.push(
				`- **${gap.gap_name}** [${gap.priority}/${gap.effort}] — ${gap.recommended_fix}`,
			);
		}
		lines.push("");
	}
	...
	if (result.goodPractices.length > 0) {
		...
		for (const p of result.goodPractices) {
			lines.push(
				`- **${p.practice_name}** (${p.effectiveness}) — ${p.description}`,
			);
		}
		lines.push("");
	}
...
export async function runOpusPipeline(
	...
	if (options.stream) {
		...
		renderPhaseComplete(
			"Auditor synthesis",
			auditorResult.assessment.top_risks.length,
			chalk.cyan,
		);
		process.stdout.write("\n");
	} else {
...
async function runAgentStreaming(
	...
	const pendingToolInputs: Map<number, { name: string; jsonStr: string }> =
		new Map();
	...
	if (verbose) {
		for await (const event of stream) {
			if (event.type === "content_block_start") {
				...
				if (pending) {
					try {
						const input = JSON.parse(pending.jsonStr) as Record<
							string,
							unknown
						>;
