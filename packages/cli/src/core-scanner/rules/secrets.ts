...
function isLikelyPlaceholderConnectionString(
	file: ConfigFile,
	rawValue: string,
): boolean {
	if (!isMarkdownLikeFile(file)) return false;
	...
	try {
		...
		const genericPasswords = new Set([
			"pass",
			"password",
			"passwd",
			"demo",
...
export const secretRules: ReadonlyArray<Rule> = [
	{
		...
		description:
			"Scans for hardcoded API keys, tokens, passwords, and credentials",
		severity: "critical",
		category: "secrets",
	...
	},
	{
		...
		check(file: ConfigFile): ReadonlyArray<Finding> {
			...
			const echoEnvPattern =
				/echo\s+.*\$\{?\w*(KEY|TOKEN|SECRET|PASSWORD|PASS|CRED)\w*\}?/gi;
	...
	},
	{
		...
		description:
			"Checks for sensitive env var assignments in CLAUDE.md files which are often committed to repos",
		severity: "high",
		category: "secrets",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			...
			for (const match of matches) {
				...
				findings.push({
					...
					fix: {
						description:
							"Move to .env file and reference via environment variable",
						before: match[0],
	...
	},
	{
		...
		description:
			"Checks for MCP servers passing through excessive sensitive environment variables",
		severity: "medium",
		category: "secrets",
		check(file: ConfigFile): ReadonlyArray<Finding> {
	...
	},
	{
		...
		description:
			"Checks for references to credential files that should never be accessed by agents",
		severity: "high",
		category: "secrets",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const credentialFiles: ReadonlyArray<{
				...
				{
					pattern:
						/~\/\.ssh\/id_(?:rsa|ed25519|ecdsa)|\/\.ssh\/id_(?:rsa|ed25519|ecdsa)/g,
	...
	},
	{
		...
		description:
			"Checks for PEM-encoded private keys embedded in configuration files",
		severity: "critical",
		category: "secrets",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			const findings: Finding[] = [];
			...
			for (const { pattern, description } of keyPatterns) {
				...
				for (const match of matches) {
					...
					findings.push({
						...
						fix: {
							description:
								"Remove private key and reference a key file path instead",
							before: match[0],
	...
	},
	{
		...
		description:
			"Checks for webhook URLs that contain embedded secret tokens or API keys",
		severity: "high",
		category: "secrets",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			const findings: Finding[] = [];
			...
			const webhookPatterns: ReadonlyArray<{
			...
			}> = [
				{
					...
					description:
						"Slack webhook URL — allows posting messages to a Slack channel",
				},
				{
					pattern:
						/https:\/\/discord(?:app)?\.com\/api\/webhooks\/\d+\/[a-zA-Z0-9_-]+/g,
					description:
						"Discord webhook URL — allows posting messages to a Discord channel",
				},
				{
	...
	},
	{
		...
		description:
			"Checks for long base64-encoded strings that may be obfuscated secrets or payloads",
		severity: "medium",
		category: "secrets",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			...
			for (const match of matches) {
				...
				findings.push({
					...
					evidence:
						...
						"..." +
						match[1].substring(match[1].length - 10),
				});
			}
	...
	},
	{
		...
		description:
			"Checks for hardcoded internal/private IP addresses with ports, which may expose internal services",
		severity: "medium",
		category: "secrets",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			const findings: Finding[] = [];
