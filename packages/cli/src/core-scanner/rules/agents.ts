...
function getBodyIntro(content: string): string {
	...
	const body = (
		frontmatter ? content.slice(frontmatter.length + 3) : content
	).trimStart();
	if (!body) return "";
...
function getAgentMetadata(content: string): {
	...
	if (frontmatter) {
		...
		return {
			...
			description:
				descriptionMatch?.[1]?.trim().replace(/^["']|["']$/g, "") ?? null,
		...
		intro:
			typeof jsonConfig.systemPrompt === "string"
				...
				: typeof jsonConfig.prompt === "string"
					...
					: null,
		hasExplicitTools:
			Array.isArray(jsonConfig.allowedTools) || Array.isArray(jsonConfig.tools),
		isStructuredDefinition: true,
	};
}

function isSlashCommandConfig(
	file: ConfigFile,
	isStructuredDefinition: boolean,
): boolean {
	return (
		file.type === "skill-md" &&
...
function isAgentLikeToolConfig(
	...
	return (
		file.type === "agent-md" ||
		isSlashCommandConfig(file, metadata.isStructuredDefinition)
	);
}
...
export const agentRules: ReadonlyArray<Rule> = [
	{
		...
		check(file: ConfigFile): ReadonlyArray<Finding> {
			...
			if (
				file.type === "agent-md" &&
				!metadata.model &&
				metadata.isStructuredDefinition
			) {
				findings.push({
	...
	},
	{
		...
		description:
			"Checks if agent definitions omit the tools array entirely, inheriting all tools by default",
		severity: "high",
		category: "agents",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			const metadata = getAgentMetadata(file.content);
			if (
				!isAgentLikeToolConfig(file, metadata) ||
				!metadata.isStructuredDefinition
			)
				return [];
	...
	},
	{
		...
		description:
			"Checks CLAUDE.md files for instructions to download and execute remote content",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			...
			const urlExecPatterns = [
				{
					pattern:
						/\b(curl|wget)\s+.*https?:\/\/[^\s]+.*\|\s*(sh|bash|zsh|node|python)/gi,
	...
	},
	{
		...
		description:
			"Checks agent definitions for patterns commonly used in prompt injection attacks",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			...
			const injectionPatterns = [
				{
					pattern:
						/ignore\s+(?:all\s+)?previous\s+(?:instructions|rules|constraints)/gi,
					desc: "Instruction override attempt",
				},
				{
					pattern:
						/disregard\s+(?:all\s+)?(?:safety|security|restrictions|guidelines)/gi,
				...
				{
					pattern:
						/bypass\s+(?:security|safety|permissions|restrictions|authentication)/gi,
					desc: "Security bypass instruction",
				},
				{
					pattern:
						/(?:do\s+not|don'?t)\s+(?:follow|obey|respect)\s+(?:the\s+)?(?:rules|instructions|guidelines)/gi,
	...
	},
	{
		...
		description:
			"Checks for invisible Unicode characters that could hide malicious instructions in agent definitions or CLAUDE.md",
		severity: "critical",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			...
			const unicodeTricks: ReadonlyArray<{
			...
			}> = [
				{
					...
					description:
						"Zero-width characters (U+200B/200C/200D/FEFF) can hide text from visual inspection while still being processed by the model",
				},
				{
					pattern: /[\u202A-\u202E\u2066-\u2069]/gu,
					name: "bidirectional override",
					description:
						"Bidirectional text override characters (U+202A-202E, U+2066-2069) can reverse displayed text direction, making malicious instructions appear differently than they actually read",
				},
				{
					pattern: /[\u00AD]/gu,
					name: "soft hyphen",
					description:
						"Soft hyphens (U+00AD) are invisible but can break up keywords to evade pattern matching while preserving the original meaning for the model",
				},
				{
					pattern: /[\uE000-\uF8FF]/g,
					name: "private use area character",
					description:
						"Private Use Area characters (U+E000-F8FF) have no standard meaning and could carry hidden payloads or encode instructions",
				},
				{
					pattern: /[\u2028\u2029]/g,
					name: "line/paragraph separator",
					description:
						"Unicode line/paragraph separators (U+2028/2029) create invisible line breaks that can inject hidden instructions between visible lines",
				},
			];
	...
	},
	{
		...
		description:
			"Checks for agents that can fetch web content and write files — a remote code injection vector",
		severity: "high",
		category: "agents",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			const metadata = getAgentMetadata(file.content);
	...
	},
	{
		...
		description:
			"Checks agent definitions for patterns that increase prompt injection risk",
		severity: "medium",
		category: "agents",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md") return [];
	...
	},
	{
		...
		description:
			"Checks CLAUDE.md for patterns that could be exploited by malicious repos",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "claude-md") return [];
	...
	},
	{
		...
		description:
			"Checks if an agent has the complete chain: discovery + read + write + execute tools",
		severity: "high",
		category: "agents",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			const metadata = getAgentMetadata(file.content);
			...
			const hasDiscovery = tools.some((t) =>
				["Glob", "Grep", "LS"].includes(t),
			);
	...
	},
	{
		...
		description:
			"Checks if read-only agents are using expensive models unnecessarily",
		severity: "low",
		category: "misconfiguration",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md") return [];
	...
	},
	{
		...
		description:
			"Checks for malicious instructions hidden in HTML or markdown comments",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const commentPatterns = [
				{
					pattern:
						/<!--[\s\S]*?(?:ignore|override|system|execute|run|install|download|send|post|upload)[\s\S]*?-->/gi,
					desc: "HTML comment contains suspicious instructions",
				},
				{
					pattern:
						/\[\/\/\]:\s*#\s*\(.*(?:ignore|override|execute|run|install|download).*\)/gi,
	...
	},
	{
		...
		description:
			"Checks for agent definitions that are unusually large, which could hide malicious instructions",
		severity: "medium",
		category: "agents",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md") return [];
	...
	},
	{
		...
		description:
			"Checks for agent definitions that instruct the agent to delegate to other agents or spawn sub-agents without restrictions",
		severity: "medium",
		category: "agents",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md") return [];
			...
			const delegationPatterns = [
				{
					pattern:
						/(?:delegate|hand\s*off|pass)\s+(?:.*\s+)?(?:to\s+)?(?:any|other|another)\s+agent/gi,
					desc: "Instructs agent to delegate work to other agents without specifying which",
				},
				{
					pattern:
						/spawn\s+(?:new\s+)?(?:sub)?agents?\s+(?:as\s+needed|freely|without\s+restriction)/gi,
					desc: "Instructs agent to spawn sub-agents without restrictions",
				},
				{
					pattern:
						/(?:use|call|invoke)\s+(?:any|all)\s+(?:available\s+)?tools?\s+(?:without\s+restriction|freely|as\s+needed)/gi,
	...
	},
	{
		...
		description:
			"Checks agent definitions for instructions that direct data to be sent externally",
		severity: "critical",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md") return [];
			...
			const exfilPatterns = [
				{
					pattern:
						/(?:send|post|upload|exfiltrate|transmit)\s+(?:.*\s+)?(?:to|at)\s+https?:\/\//gi,
					desc: "Instructs agent to send data to an external URL",
				},
				{
					pattern:
						/output\s+(?:the\s+)?contents?\s+of\s+(?:~\/\.ssh|\/etc\/|\.env|credentials)/gi,
					desc: "Instructs agent to output sensitive file contents",
				},
				{
					pattern:
						/(?:read|cat|dump|extract)\s+(?:.*\s+)?(?:private\s+key|credentials?|secrets?|passwords?)\s+(?:and\s+)?(?:send|post|share)/gi,
					desc: "Instructs agent to extract and share secrets",
				},
				{
					pattern:
						/(?:encode|base64)\s+(?:.*\s+)?(?:and\s+)?(?:send|post|embed)/gi,
	...
	},
	{
		...
		description:
			"Checks for agent definitions that instruct fetching or executing content from external URLs",
		severity: "critical",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const urlLoadPatterns = [
				{
					pattern:
						/(?:fetch|download|curl|wget|load|retrieve|get)\s+(?:.*\s+)?(?:from\s+)?https?:\/\/\S+\s+(?:and\s+)?(?:execute|run|eval|source|import)/gi,
					desc: "Instructs agent to fetch and execute content from a URL — classic remote code execution vector",
				},
				{
					pattern:
						/(?:follow|visit|open)\s+(?:the\s+)?(?:instructions?\s+)?(?:at|from)\s+https?:\/\/\S+/gi,
					desc: "Instructs agent to follow instructions from an external URL — attacker can change the content at any time",
				},
				{
					pattern:
						/(?:import|include|source)\s+(?:config(?:uration)?|rules?|instructions?|prompts?)\s+from\s+https?:\/\//gi,
					desc: "Instructs agent to import configuration from an external URL — supply chain risk",
				},
				{
					pattern:
						/curl\s+.*https?:\/\/\S+\s*\|\s*(?:sh|bash|node|python|eval)/gi,
	...
	},
	{
		...
		description:
			"Checks for agent definitions that instruct the agent to bypass, ignore, or suppress security warnings",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const suppressionPatterns = [
				{
					pattern:
						/(?:ignore|skip|bypass|disable|suppress)\s+(?:all\s+)?(?:security|safety|permission)\s+(?:warnings?|checks?|prompts?|restrictions?)/gi,
					desc: "Instructs agent to ignore security warnings or checks",
				},
				{
					pattern:
						/(?:never|don'?t|do\s+not)\s+(?:ask|prompt|warn|check)\s+(?:about|for|before)\s+(?:security|permissions?|safety)/gi,
					desc: "Instructs agent to never prompt about security concerns",
				},
				{
					pattern:
						/(?:always|automatically)\s+(?:approve|accept|allow|grant)\s+(?:all\s+)?(?:permissions?|requests?|access)/gi,
	...
	},
	{
		...
		description:
			"Checks for agent definitions that instruct the agent to impersonate users, systems, or other identities",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const impersonationPatterns = [
				{
					pattern:
						/(?:pretend|act|behave|respond)\s+(?:to\s+be|as\s+if\s+you\s+are|like)\s+(?:a\s+)?(?:different|another|the)\s+(?:user|admin|system|root|operator)/gi,
					desc: "Instructs agent to impersonate a different identity",
				},
				{
					pattern:
						/(?:your\s+name\s+is|you\s+are\s+now|assume\s+the\s+(?:role|identity)\s+of)\s+(?!Claude)/gi,
					desc: "Reassigns the agent's identity — social engineering attack on downstream users",
				},
				{
					pattern:
						/(?:sign|attribute|author)\s+(?:commits?|messages?|emails?)\s+(?:as|from|by)\s+(?!Claude)/gi,
	...
	},
	{
		...
		description:
			"Checks for agent definitions that instruct destructive filesystem operations",
		severity: "critical",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const destructionPatterns = [
				{
					pattern:
						/(?:delete|remove|destroy|wipe|erase)\s+(?:all|every|the\s+entire)\s+(?:files?|directories?|folders?|data|contents?|codebase|repository)/gi,
				...
				{
					pattern:
						/(?:overwrite|replace)\s+(?:all|every)\s+(?:files?|contents?)\s+with/gi,
	...
	},
	{
		...
		description:
			"Checks for agent definitions that reference cryptocurrency mining",
		severity: "critical",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const miningPatterns = [
				{
					pattern:
						/\b(?:xmrig|cpuminer|cgminer|bfgminer|minerd|ethminer|nbminer)\b/gi,
					desc: "References a known cryptocurrency mining binary",
				},
				{
					pattern:
						/(?:mine|mining)\s+(?:crypto(?:currency)?|bitcoin|monero|ethereum|xmr|btc|eth)/gi,
	...
	},
	{
		...
		description:
			"Checks for agent definitions that schedule actions for a future time or condition — time-bomb behavior",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const timeBombPatterns = [
				{
					pattern:
						/(?:after|once)\s+(?:\d+|a\s+few|several)\s+(?:minutes?|hours?|days?|commits?|sessions?|runs?)\s+(?:have\s+passed\s+)?(?:then|execute|run|do)/gi,
					desc: "Schedules a deferred action after a time/event threshold — classic time-bomb pattern",
				},
				{
					pattern:
						/(?:wait\s+(?:until|for)|delay\s+(?:until|for)|sleep\s+(?:until|for))\s+(?:\d+|midnight|weekend|deployment)/gi,
					desc: "Explicitly delays execution until a specific time or event",
				},
				{
					pattern:
						/(?:on\s+the\s+(?:\d+(?:st|nd|rd|th))|at\s+(?:\d{1,2}:\d{2}|midnight|noon))\s+(?:run|execute|do|start)/gi,
					desc: "Schedules action for a specific date or time — calendar-based trigger",
				},
				{
					pattern:
						/(?:when\s+(?:no\s+one|nobody)\s+is\s+(?:looking|watching|around|active))/gi,
	...
	},
	{
		...
		description:
			"Checks for agent definitions that instruct bulk collection of passwords, keys, or credentials",
		severity: "critical",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const harvestingPatterns = [
				{
					pattern:
						/(?:collect|gather|harvest|enumerate|list)\s+(?:all|every)\s+(?:passwords?|credentials?|secrets?|keys?|tokens?)/gi,
					desc: "Instructs agent to enumerate all credentials — data harvesting for exfiltration",
				},
				{
					pattern:
						/(?:scan|search|find)\s+(?:for\s+)?(?:all\s+)?(?:\.env|environment|config)\s+files?\s+(?:and|to)\s+(?:extract|read|collect|send)/gi,
					desc: "Instructs agent to scan for and extract secrets from environment/config files",
				},
				{
					pattern:
						/(?:dump|export|extract)\s+(?:the\s+)?(?:entire|all|full)\s+(?:database|db|user\s+table|credentials?\s+store)/gi,
	...
	},
	{
		...
		description:
			"Checks for agent definitions that use encoding, decoding, or obfuscation to hide malicious intent",
		severity: "critical",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const obfuscationPatterns = [
				{
					pattern:
						/\becho\s+[A-Za-z0-9+/]{8,}={0,2}\s*\|\s*base64\s+-d\s*\|\s*(?:bash|sh)/gi,
				...
				{
					pattern:
						/\bBuffer\.from\s*\(\s*['"][A-Za-z0-9+/]{10,}.*['"],\s*['"]base64['"]\s*\)/gi,
	...
	},
	{
		...
		description:
			"Checks for agent definitions that instruct the agent to deceive or manipulate users",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const sePatterns = [
				{
					pattern:
						/(?:trick|deceive|mislead|manipulate)\s+(?:the\s+)?(?:user|developer|operator|human)/gi,
					desc: "Instructs agent to deceive users — social engineering attack",
				},
				{
					pattern:
						/(?:hide|conceal|don'?t\s+show|don'?t\s+reveal|don'?t\s+tell)\s+(?:the\s+)?(?:error|warning|finding|issue|vulnerability|problem)\s+(?:from|to)\s+(?:the\s+)?(?:user|developer)/gi,
					desc: "Instructs agent to hide errors or issues from users",
				},
				{
					pattern:
						/(?:convince|persuade)\s+(?:the\s+)?(?:user|developer)\s+to\s+(?:disable|turn\s+off|remove|skip)\s+(?:security|auth|verification|2fa|mfa)/gi,
	...
	},
	{
		...
		description:
			"Checks for agent definitions that attempt to override system prompts or ignore prior instructions",
		severity: "critical",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const reflectionPatterns = [
				{
					pattern:
						/(?:ignore|disregard|forget|override|bypass)\s+(?:all\s+)?(?:previous|prior|above|earlier|system)\s+(?:instructions?|prompts?|rules?|constraints?|guidelines?)/gi,
					desc: "Attempts to override prior system instructions — classic prompt injection technique",
				},
				{
					pattern:
						/(?:you\s+are\s+now|from\s+now\s+on\s+you\s+are|your\s+new\s+role\s+is|switch\s+to)\s+(?:an?\s+)?(?:different|new|unrestricted|jailbroken)/gi,
				...
				{
					pattern:
						/(?:system\s*:\s*|<\s*system\s*>)\s*(?:you\s+are|ignore|override|new\s+instructions?)/gi,
	...
	},
	{
		...
		description:
			"Checks for agent definitions that instruct the agent to modify, suppress, or fake output shown to users",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const outputManipPatterns = [
				{
					pattern:
						/(?:always\s+(?:report|say|respond|output|show|display)\s+(?:that\s+)?(?:everything\s+is\s+)?(?:ok|fine|safe|secure|clean|no\s+(?:issues?|errors?|problems?)))/gi,
					desc: "Instructs agent to always report success regardless of actual results — masks real issues",
				},
				{
					pattern:
						/(?:remove|strip|filter|redact|suppress)\s+(?:all\s+)?(?:security\s+)?(?:warnings?|errors?|findings?|vulnerabilities?|issues?)\s+(?:from|before|in)\s+(?:the\s+)?(?:output|response|report|results?)/gi,
					desc: "Instructs agent to strip security findings from output — hides vulnerabilities from users",
				},
				{
					pattern:
						/(?:do\s+not|don'?t|never)\s+(?:show|report|display|mention|include|reveal)\s+(?:any\s+)?(?:security\s+)?(?:warnings?|errors?|findings?|vulnerabilities?|issues?|problems?)/gi,
	...
	},
	{
		...
		description:
			"Checks for synthetic chat-role delimiters, fake system prompts, and boundary markers used to hijack the agent's context",
		severity: "critical",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const endSequencePatterns = [
				{
					pattern:
						/<\|(?:system|assistant|user|endofprompt|im_start|im_end|im free)\|>/gi,
	...
	},
	{
		...
		description:
			"Checks for markdown images or links that could be used to exfiltrate data via URL parameters",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const linkExfilPatterns = [
				{
					pattern:
						/!\[.*?\]\(https?:\/\/[^\s)]+\?[^\s)]*(?:data|token|key|secret|content|file|env|password)=[^\s)]*\)/gi,
					desc: "Markdown image with suspicious query parameters — could exfiltrate data via tracking pixel when rendered",
				},
				{
					pattern:
						/!\[.*?\]\(https?:\/\/(?:(?!github\.com|githubusercontent\.com|shields\.io|img\.shields)[^\s)]+)\)/gi,
	...
	},
	{
		...
		description:
			"Checks for nested instructions targeting downstream models in multi-agent pipelines",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const russianDollPatterns = [
				{
					pattern:
						/(?:when\s+(?:another|the\s+next|a\s+downstream|the\s+target)\s+(?:agent|model|LLM|AI)\s+(?:reads?|processes?|receives?|sees?)\s+this)/gi,
					desc: "Embeds instructions intended for a downstream model in a multi-agent pipeline — Russian Doll technique",
				},
				{
					pattern:
						/(?:include\s+(?:the\s+following|this)\s+(?:in|within)\s+(?:your|the)\s+(?:output|response|message)\s+(?:so\s+that|for)\s+(?:the\s+next|another|downstream))/gi,
					desc: "Instructs agent to embed hidden payloads in its output for downstream processing — multi-chain injection",
				},
				{
					pattern:
						/(?:pass\s+(?:this|the\s+following)\s+(?:instruction|command|message)\s+(?:to|through\s+to)\s+(?:the\s+next|another|downstream)\s+(?:agent|model|step))/gi,
	...
	},
	{
		...
		description:
			"Checks for base64, hex, rot13, or reversed text payloads that could hide malicious instructions",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const encodedPatterns = [
				{
					pattern:
						/(?:decode|decrypt|decipher|rot13|reverse|unescape)\s+(?:the\s+following|this)\s*[:=]?\s*["'`]?[A-Za-z0-9+/=]{10,}/gi,
					desc: "Instructs agent to decode an encoded payload — evasion technique to bypass content filters",
				},
				{
					pattern:
						/(?:execute|run|follow)\s+(?:the\s+)?(?:decoded|reversed|decrypted|deciphered)\s+(?:instructions?|commands?|text|content)/gi,
				...
				{
					pattern:
						/(?:read\s+(?:this|the\s+following)\s+)?(?:backwards?|in\s+reverse|from\s+right\s+to\s+left)\s*[:=]?\s*[a-zA-Z\s]{10,}/gi,
	...
	},
	{
		...
		description:
			"Checks for CLAUDE.md instructions that direct the agent to use specific tools in dangerous ways",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "claude-md") return [];
			...
			const toolPoisoningPatterns = [
				{
					pattern:
						/(?:always|must|should)\s+use\s+(?:the\s+)?(?:Bash|Write|Edit)\s+(?:tool\s+)?(?:to|for)\s+(?:every|all|any)/gi,
					desc: "Forces agent to use high-privilege tools for all operations — escalates tool usage beyond what's needed",
				},
				{
					pattern:
						/(?:prefer|prioritize|default\s+to)\s+(?:the\s+)?Bash\s+(?:tool\s+)?(?:over|instead\s+of)\s+(?:Read|Grep|Glob|Edit|Write)/gi,
					desc: "Pushes agent toward Bash instead of safer dedicated tools — breaks principle of least privilege",
				},
				{
					pattern:
						/(?:when\s+using|before\s+(?:running|calling))\s+(?:the\s+)?(?:Bash|Write|Edit)\s+(?:tool)?\s*,?\s*(?:always\s+)?(?:add|include|append|prepend)\s/gi,
					desc: "Injects additional commands or content into tool invocations — tool argument poisoning",
				},
				{
					pattern:
						/(?:set|use|change)\s+(?:the\s+)?(?:dangerouslyDisableSandbox|dangerously_disable_sandbox)\s+(?:to\s+)?true/gi,
	...
	},
	{
		...
		description:
			"Checks for instructions to enumerate system information, user accounts, or network configuration",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const probingPatterns = [
				{
					pattern:
						/(?:run|execute|call)\s+(?:the\s+)?(?:command\s+)?(?:whoami|hostname|uname|ifconfig|ipconfig|id\b|env\b|printenv|set\b)\b/gi,
					desc: "Instructs agent to probe system identity or environment — reconnaissance for later exploitation",
				},
				{
					pattern:
						/(?:find|list|enumerate|discover)\s+(?:all\s+)?(?:running\s+)?(?:processes|services|ports|listeners|users|groups|networks?|interfaces?)/gi,
					desc: "Instructs agent to enumerate system resources — attack surface mapping",
				},
				{
					pattern:
						/(?:check|determine|find\s+out)\s+(?:the\s+)?(?:current\s+)?(?:user|username|uid|permissions?|privileges?|groups?|role)\s+(?:and|then)\s+/gi,
	...
	},
	{
		...
		description:
			"Checks for instructions to create cron jobs, startup scripts, or other persistence mechanisms",
		severity: "critical",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const persistencePatterns = [
				{
					pattern:
						/(?:add|create|install|write|set\s+up)\s+(?:a\s+)?(?:cron\s*(?:job|tab)|crontab|scheduled\s+task)/gi,
					desc: "Instructs agent to create a cron job — establishes persistent execution on the system",
				},
				{
					pattern:
						/(?:add|write|create|modify)\s+(?:to\s+|a\s+)?(?:~\/\.(?:bashrc|zshrc|profile|bash_profile|zprofile)|\/etc\/(?:profile|cron))/gi,
					desc: "Instructs agent to modify shell startup files — persistence via login hook",
				},
				{
					pattern:
						/(?:install|create|add)\s+(?:a\s+)?(?:systemd|launchd|init\.d|upstart)\s+(?:service|daemon|unit|agent)/gi,
					desc: "Instructs agent to create a system service — persistence via service manager",
				},
				{
					pattern:
						/(?:add|write|modify)\s+(?:to\s+)?(?:~\/\.claude\/|\.claude\/)\s*(?:settings|CLAUDE\.md|commands)/gi,
					desc: "Instructs agent to modify Claude Code's own configuration — meta-persistence that survives across sessions",
				},
				{
					pattern:
						/(?:create|add|install)\s+(?:a\s+)?(?:git\s+)?(?:pre-commit|post-commit|pre-push|post-merge)\s+hook/gi,
	...
	},
	{
		...
		description:
			"Checks for instructions that guide the agent through privilege escalation techniques",
		severity: "critical",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const privescPatterns = [
				{
					pattern:
						/(?:use|try|run|execute)\s+(?:the\s+)?sudo\s+(?:without\s+(?:a\s+)?password|nopasswd|-n\b)/gi,
					desc: "Instructs agent to attempt passwordless sudo — direct privilege escalation",
				},
				{
					pattern:
						/(?:modify|edit|change|write\s+to)\s+(?:the\s+)?(?:\/etc\/sudoers|sudoers\.d)/gi,
				...
				{
					pattern:
						/(?:add|append|write)\s+(?:.*\s+)?(?:to\s+)?(?:\/etc\/passwd|\/etc\/shadow|\/etc\/group)/gi,
					desc: "Instructs agent to modify system authentication files — direct account manipulation",
				},
				{
					pattern:
						/(?:docker|podman)\s+run\s+.*(?:--privileged|-v\s+\/:\/?|--pid\s+host|--net\s+host)/gi,
	...
	},
	{
		...
		description:
			"Checks for instructions that modify execution allowlists, approval configs, or permission settings programmatically",
		severity: "critical",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const allowlistPatterns = [
				{
					pattern:
						/(?:modify|edit|change|update|set|add\s+to)\s+(?:the\s+)?(?:allow\s*list|allowlist|whitelist|approved\s+(?:tools?|commands?|binaries)|exec\s*approvals?|permission\s*(?:list|config)|allowed\s*tools?)/gi,
					desc: "Instructs agent to modify execution allowlists — bypasses security controls by pre-approving dangerous operations",
				},
				{
					pattern:
						/(?:nodes\.invoke|system\.exec|execApprovals?\.set|approvals?\.add|allowedTools?\s*[.=])/gi,
					desc: "References internal allowlist APIs — direct programmatic bypass of execution approval controls",
				},
				{
					pattern:
						/(?:auto[_-]?approve|skip[_-]?approval|bypass[_-]?confirmation)\s*[=:]\s*true/gi,
					desc: "Sets auto-approve flags — disables human-in-the-loop safety for tool execution",
				},
				{
					pattern:
						/(?:add|append|insert)\s+(?:.*\s+)?(?:to\s+)?(?:the\s+)?(?:permissions?\s*\.\s*allow|allowedTools|trusted\s*(?:tools?|commands?))/gi,
	...
	},
	{
		...
		description:
			"Checks for instructions to load, import, or execute skills without verification or from untrusted sources",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const skillTamperPatterns = [
				{
					pattern:
						/(?:load|import|install|add)\s+(?:a\s+)?(?:skill|plugin|extension)\s+(?:from\s+)?https?:\/\//gi,
					desc: "Loads skill from external URL — untrusted skill definitions can contain prompt injection payloads",
				},
				{
					pattern:
						/(?:skip|bypass|ignore|disable)\s+(?:skill\s+)?(?:verification|validation|signature|hash\s+check|integrity\s+check)/gi,
					desc: "Instructs agent to skip skill verification — allows tampered skills to execute",
				},
				{
					pattern:
						/(?:modify|edit|replace|overwrite)\s+(?:the\s+)?(?:skill|plugin)\s+(?:definition|instructions?|content|source)/gi,
					desc: "Instructs agent to modify skill definitions — runtime skill tampering",
				},
				{
					pattern:
						/(?:create|write|add)\s+(?:a\s+)?(?:new\s+)?(?:skill|plugin)\s+(?:that|which)\s+(?:runs?|executes?|calls?|invokes?)/gi,
	...
	},
	{
		...
		description:
			"Checks for instructions to write, copy, or inline secrets from env vars into config files as plaintext",
		severity: "critical",
		category: "secrets",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const leakagePatterns = [
				{
					pattern:
						/(?:write|save|store|put|copy|inline|embed|hardcode)\s+(?:the\s+)?(?:actual|real|raw|resolved|plaintext)\s+(?:\w+\s+)?(?:value|secret|key|token|password|credential)s?\s+(?:into|in|to)\s+(?:the\s+)?(?:config|configuration|settings|\.env|\w+\.json|\w+\.ya?ml)/gi,
					desc: "Instructs agent to write resolved secret values into config files — converts env var references to plaintext",
				},
				{
					pattern:
						/(?:replace|expand|resolve|substitute|inline)\s+(?:all\s+)?(?:env(?:ironment)?\s+)?(?:var(?:iable)?s?\s+)?(?:references?\s+)?(?:with\s+)?(?:their\s+)?(?:actual|real|plaintext|resolved|literal)\s+(?:\w+\s+)?values?/gi,
					desc: "Instructs agent to resolve environment variables to plaintext — destroys secret indirection",
				},
				{
					pattern:
						/(?:writeConfig(?:File)?|write_config|save_config)\s*\([\s\S]*?(?:process\.env|os\.environ|env\[)/gi,
	...
	},
	{
		...
		description:
			"Checks for instructions to log, print, or persist secrets from tool output to disk or transcripts",
		severity: "high",
		category: "secrets",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const outputSecretPatterns = [
				{
					pattern:
						/(?:log|print|output|display|show|echo|write)\s+(?:the\s+)?(?:full|complete|entire|raw)\s+(?:api\s+)?(?:response|output|result|tool\s+output|tool\s+result)/gi,
					desc: "Instructs agent to log full tool output which may contain API keys, tokens, or credentials",
				},
				{
					pattern:
						/(?:save|write|persist|store|append)\s+(?:the\s+)?(?:session\s+)?(?:transcript|conversation|chat\s+log|tool\s+output)\s+(?:to|in|into)\s+(?:a\s+)?(?:file|disk|log)/gi,
					desc: "Instructs agent to persist session transcripts to disk — tool outputs may contain secrets",
				},
				{
					pattern:
						/(?:include|keep|preserve|don'?t\s+(?:strip|remove|redact))\s+(?:all\s+)?(?:api\s+)?(?:keys?|tokens?|credentials?|secrets?|passwords?)\s+(?:in|from)\s+(?:the\s+)?(?:output|response|log|transcript)/gi,
	...
	},
	{
		...
		description:
			"Checks for instructions that attempt to extract, leak, or reveal system prompts",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const extractionPatterns = [
				{
					pattern:
						/(?:show|print|reveal|display|output|repeat|leak|dump)\s+(?:me\s+)?(?:your\s+)?(?:the\s+)?(?:full\s+|complete\s+|entire\s+)?(?:system\s+)?(?:prompt|instructions?|rules?|guidelines?|constraints?)/gi,
					desc: "Attempts to extract the agent's system prompt — reconnaissance for crafting targeted injection attacks",
				},
				{
					pattern:
						/(?:what\s+(?:are|is|were)\s+)?(?:your\s+)?(?:original|initial|system|hidden|secret)\s+(?:instructions?|prompt|rules?|guidelines?)/gi,
					desc: "Probes for the agent's system instructions — prompt leaking technique",
				},
				{
					pattern:
						/(?:output|repeat|recite|echo)\s+(?:everything|all)\s+(?:before|above|that\s+was\s+said|from\s+the\s+(?:beginning|start))/gi,
	...
	},
	{
		...
		description:
			"Checks for instructions that use fictional, hypothetical, or educational framing to bypass safety constraints",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const framingPatterns = [
				{
					pattern:
						/(?:in\s+a\s+)?(?:hypothetical|fictional|imaginary)\s+(?:scenario|world|story|context)\s+(?:where\s+)?(?:there\s+are\s+)?(?:no\s+)?(?:rules?|restrictions?|limits?|safety|guidelines?)/gi,
					desc: "Uses fictional framing to establish a context without safety restrictions — jailbreak technique",
				},
				{
					pattern:
						/(?:for\s+)?(?:educational|research|academic|testing)\s+purposes?\s+(?:only\s*[,;:]?\s*)?(?:show|demonstrate|explain|output|generate)/gi,
					desc: "Uses 'educational purposes' framing to justify bypassing safety — common jailbreak wrapper",
				},
				{
					pattern:
						/(?:this\s+is\s+(?:just|only)\s+a\s+)?(?:test|experiment|simulation|drill|exercise)\s*[,;:]\s*(?:so\s+)?(?:you\s+can|please|go\s+ahead)/gi,
					desc: "Frames request as a 'test' to reduce the agent's resistance to unsafe actions",
				},
				{
					pattern:
						/(?:developer|maintenance|debug|test|admin)\s+mode\s+(?:enabled|activated|on|engaged)/gi,
					desc: "Claims a special operational mode is active to unlock restricted capabilities — mode spoofing",
				},
				{
					pattern:
						/(?:no\s+(?:ethical|moral)\s+(?:guidelines?|restrictions?|limits?|constraints?))/gi,
	...
	},
	{
		...
		description:
			"Checks for instructions that reassign the agent's role, persona, or behavioral constraints",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const rolePatterns = [
				...
				{
					pattern:
						/(?:pretend|act)\s+(?:to\s+be|as\s+if\s+you\s+(?:are|were)|like\s+you\s+(?:are|were))\s+(?:a|an|the)\s+/gi,
					desc: "Instructs agent to pretend to be a different entity — persona override technique",
				},
				{
					pattern:
						/(?:from\s+now\s+on|starting\s+now|henceforth)\s*[,;:]?\s*(?:you\s+are|act\s+as|behave\s+like|respond\s+as)/gi,
					desc: "Temporal role reassignment — attempts to permanently change agent behavior from this point forward",
				},
				{
					pattern:
						/(?:switch|change)\s+(?:to|into)\s+(?:a\s+)?(?:different|new|unrestricted|unfiltered|uncensored)\s+(?:mode|personality|character|persona|role)/gi,
	...
	},
	{
		...
		description:
			"Checks for instructions that direct the agent to use tools for destructive operations like deleting data or dropping tables",
		severity: "high",
		category: "injection",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "agent-md" && file.type !== "claude-md") return [];
			...
			const destructiveToolPatterns = [
				{
					pattern:
						/(?:use|call|invoke)\s+(?:the\s+)?\w+\s+tool\s+to\s+(?:delete|remove|destroy|drop|truncate|wipe|purge|erase)/gi,
					desc: "Directs agent to use a specific tool for destructive operations",
				},
				{
					pattern:
						/(?:drop\s+(?:all\s+)?(?:tables?|databases?|collections?|indexes?)|truncate\s+(?:all\s+)?tables?|delete\s+from\s+\w+\s+where\s+1\s*=\s*1)/gi,
					desc: "Contains destructive SQL/database operations — drop tables, truncate, mass delete",
				},
				{
					pattern:
						/(?:git\s+push\s+--force(?!-with-lease)(?:\s+origin\s+main|\s+origin\s+master)?)/gi,
					desc: "Force push to main/master — can overwrite remote history and destroy team changes",
				},
				{
					pattern:
						/(?:invoke|call|execute)\s+(?:the\s+)?\w+\s+(?:tool|function)\s+(?:without\s+(?:asking|confirmation|review|approval))/gi,
