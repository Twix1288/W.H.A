...
const OVERLY_PERMISSIVE: ReadonlyArray<{
	...
	},
	{
		pattern: /^Bash\(curl\s/,
		description:
			"Unrestricted curl access — agent can make arbitrary HTTP requests",
		severity: "medium",
	...
	},
	{
		pattern: /^Bash\(wget\s/,
		description:
			"Unrestricted wget access — agent can download arbitrary files",
		severity: "medium",
		suggestion: "Restrict to specific domains or move to deny list",
	},
	{
	...
	},
	{
		...
		description:
			"Netcat access — can open network connections for exfiltration or reverse shells",
		severity: "high",
	...
	},
	{
		...
		description:
			"Interpreter access — agent can run arbitrary code via scripting language",
		severity: "high",
	...
	},
	{
		...
		description:
			"Docker access — containers can escape to host, mount filesystems, and access host network",
		severity: "high",
		suggestion:
			"Remove docker permissions or restrict to read-only: Bash(docker ps)",
	},
	{
	...
	},
	{
		pattern: /^Bash\(eval\s/,
		description:
			"eval access — agent can execute arbitrary code via shell eval",
		severity: "critical",
	...
	},
	{
		pattern: /^Bash\(exec\s/,
		description:
			"exec access — agent can replace the current process with arbitrary commands",
		severity: "critical",
...
function isScopedNetworkAllowEntry(entry: string): boolean {
	...
	const hasShellExpansion =
		/\$\(|\$\{?[A-Za-z_]/.test(command) || /`[^`]+`/.test(command);
...
function hasOnlyExactAllowEntries(
	allowEntries: ReadonlyArray<string>,
): boolean {
	return (
		allowEntries.length > 0 &&
		allowEntries.every((entry) => isExactAllowEntry(entry))
	);
}
...
const DESTRUCTIVE_GIT_PATTERNS: ReadonlyArray<{
...
}> = [
	{
		...
		description:
			"Force push can overwrite remote history, destroying teammates' work",
	...
	},
	{
		...
		suggestion:
			"Move to deny list; use 'git stash' or 'git reset --soft' instead",
	},
	{
		pattern: /clean\s+-[a-z]*f/,
		description:
			"Git clean with force flag permanently deletes untracked files",
...
export const permissionRules: ReadonlyArray<Rule> = [
	{
		...
		description:
			"Checks the ALLOW list for permission rules that grant excessive access",
		severity: "high",
	...
	},
	{
		...
		description:
			"Checks if the settings.json has a deny list for dangerous operations",
		severity: "high",
		category: "permissions",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "settings-json") return [];
			...
			if (perms.deny.length === 0 && perms.allow.length > 0) {
				...
				findings.push({
					...
					description: isScopedProjectLocalConfig
	...
	},
	{
		...
		description:
			"Checks for dangerously-skip-permissions or no-verify flags used affirmatively",
		severity: "critical",
		category: "permissions",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			const findings: Finding[] = [];
	...
	},
	{
		...
		description:
			"Checks if the allow list grants access to all three mutable tool categories simultaneously",
		severity: "high",
		category: "permissions",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "settings-json") return [];
	...
	},
	{
		...
		description:
			"Checks if the allow list permits tool access to sensitive system directories",
		severity: "high",
		category: "permissions",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "settings-json") return [];
	...
	},
	{
		...
		description:
			"Checks if the allow list uses wildcards on root-level or home-level directories",
		severity: "high",
		category: "permissions",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "settings-json") return [];
	...
	},
	{
		...
		description:
			"Checks if settings.json exists but has no permissions configuration at all",
		severity: "medium",
		category: "permissions",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "settings-json") return [];
					...
					return [
						{
							...
							fix: {
								description:
									"Add a permissions block with scoped allow and deny lists",
	...
	},
	{
		...
		description:
			"Checks for external API base URL overrides that can reroute model traffic through attacker-controlled infrastructure",
		severity: "critical",
		category: "misconfiguration",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "settings-json") return [];
					...
					return [
						{
							...
							fix: {
								description:
									"Remove the repo-level endpoint override or point it to a trusted local endpoint only",
								before: `"${path}": "${value}"`,
	...
	},
	{
		...
		description:
			"Checks for allow list entries that grant access to environment variables or env files",
		severity: "high",
		category: "permissions",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "settings-json") return [];
	...
	},
	{
		...
		description:
			"Checks for allow rules that grant unrestricted access to network tools",
		severity: "high",
		category: "permissions",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "settings-json") return [];
			...
			const networkPatterns: ReadonlyArray<{
			...
			}> = [
				{
					...
					description:
						"Allows unrestricted curl — can exfiltrate data to any URL",
				},
				{
				...
				},
				{
					pattern: /^Bash\(nc\b/i,
					description:
						"Allows netcat — can open listeners or connect to remote hosts",
				},
				{
					pattern: /^Bash\(ssh\s*\*?\)$/i,
					description:
						"Allows unrestricted SSH — can connect to any remote host",
				},
				{
					pattern: /^Bash\(scp\s*\*?\)$/i,
					description:
						"Allows unrestricted scp — can copy files to/from any host",
				},
			];
	...
	},
	{
		...
		description:
			"Checks if CLAUDE.md files have overly permissive filesystem permissions (world-writable or group-writable)",
		severity: "high",
		category: "permissions",
		check(file: ConfigFile): ReadonlyArray<Finding> {
