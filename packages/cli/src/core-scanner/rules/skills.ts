...
export const skillRules: ReadonlyArray<Rule> = [
	{
		...
		description:
			"Checks whether SKILL.md files define observation and feedback hooks for self-improvement loops",
		severity: "medium",
		category: "skills",
		check(file, allFiles = []) {
	...
	},
	{
		...
		description:
			"Checks whether SKILL.md files define versioning and rollback metadata",
		severity: "medium",
		category: "skills",
		check(file, allFiles = []) {
			if (!isSkillDefinitionFile(file)) return [];
