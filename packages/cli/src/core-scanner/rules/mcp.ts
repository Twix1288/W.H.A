...
const MCP_RISK_PROFILES: ReadonlyArray<{
	...
	},
	{
		...
		recommendation:
			"Restrict to specific domains and disable script running where possible",
	},
	{
		...
		recommendation:
			"Use allowlist of specific commands instead of unrestricted shell access",
	},
	{
	...
	},
	{
		...
		recommendation:
			"Restrict to specific channels and require confirmation for sends",
	},
];
...
function finalizeMcpFindings(
	...
	return findings.map((finding) => {
		...
		if (
			baseFinding.category !== "mcp" &&
			baseFinding.category !== "misconfiguration"
		) {
			return baseFinding;
		}
...
function isScopedFilesystemServer(
	name: string,
...
const rawMcpRules: ReadonlyArray<Rule> = [
	{
		...
		description:
			"Checks MCP server configs for servers that grant excessive capabilities",
		severity: "high",
		category: "mcp",
				...
				for (const [name, server] of Object.entries(servers)) {
					...
					for (const profile of MCP_RISK_PROFILES) {
						if (profile.namePattern.test(name)) {
							const severity =
								profile.namePattern.test(name) &&
								isScopedFilesystemServer(name, serverConfig)
									? "medium"
									: profile.risk;
							const description =
	...
	},
	{
		...
		description:
			"Checks for enableAllProjectMcpServers=true which silently trusts project-defined MCP servers",
		severity: "critical",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
			...
			try {
				...
				return enabledPaths.map((path, index) => ({
					...
					fix: {
						description:
							"Disable project-wide MCP auto-approval and review each server explicitly",
						before: `"${path}": true`,
						after: `"${path}": false`,
						auto: false,
					},
	...
	},
	{
		...
		description:
			"Checks if MCP configs have hardcoded secrets instead of env var references",
		severity: "critical",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
	...
	},
	{
		...
		description:
			"Checks for MCP servers using npx -y which auto-installs packages without confirmation",
		severity: "medium",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json") return [];
	...
	},
	{
		...
		description:
			"Checks for MCP servers with filesystem access to root or home directory",
		severity: "high",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
	...
	},
	{
		...
		description:
			"Checks for MCP servers using URL-based transport connecting to external hosts",
		severity: "high",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
		...
		check(file: ConfigFile): ReadonlyArray<Finding> {
			...
			try {
				...
				for (const [name, server] of Object.entries(servers)) {
					...
					if (
						/\b(curl|wget)\b.*\|\s*(sh|bash|zsh|node|python)/i.test(fullCommand)
					) {
						findings.push({
							...
							fix: {
								...
								after:
									"Install the package locally with npm/pip and reference it directly",
								auto: false,
							},
						});
						continue;
					...
					const hasRemoteUrl = args.some((a) =>
						/^https?:\/\/.+\.(sh|py|js|ts|exe|bin)$/i.test(a),
					);
					if (
						hasRemoteUrl &&
						...
						findings.push({
							...
							fix: {
								description:
									"Download the script locally and reference the local copy",
								before: fullCommand.substring(0, 60),
	...
	},
	{
		...
		description:
			"Checks for shell metacharacters in MCP server arguments that could enable command injection",
		severity: "medium",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
					...
					for (const arg of args) {
						...
						if (shellMetachars.test(arg)) {
							findings.push({
								...
								fix: {
									description:
										"Split into separate arguments without shell metacharacters",
	...
	},
	{
		...
		description:
			"Checks for MCP servers that override system-critical environment variables like PATH or LD_PRELOAD",
		severity: "critical",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
			...
			try {
				...
				const dangerousEnvVars: ReadonlyArray<{
				...
				}> = [
					{
						name: "PATH",
						description:
							"Controls which executables are found — can redirect to malicious binaries",
					},
					{
						name: "LD_PRELOAD",
						description:
							"Injects shared libraries into every process — classic privilege escalation",
					},
					{
						name: "LD_LIBRARY_PATH",
						description:
							"Redirects dynamic library loading — can intercept system calls",
					},
					{
						name: "NODE_OPTIONS",
						description:
							"Injects flags into every Node.js process — can load arbitrary code",
					},
					{
						name: "PYTHONPATH",
						description:
							"Redirects Python module imports — can load malicious modules",
					},
					{
						name: "HOME",
						description:
							"Changes home directory — can redirect config file loading",
					},
				];
	...
	},
	{
		...
		description:
			"Checks for MCP servers that use sh/bash -c as command, which defeats argument separation safety",
		severity: "high",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
				...
				for (const [name, server] of Object.entries(servers)) {
					...
					if (/^(sh|bash|zsh|cmd)$/.test(command) && args.includes("-c")) {
						findings.push({
							...
							fix: {
								description:
									"Use the target binary directly instead of wrapping in sh -c",
	...
	},
	{
		...
		description:
			"Checks for MCP servers installed from git URLs which are mutable supply chain risks",
		severity: "high",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
					...
					for (const arg of args) {
						if (/git\+https?:\/\/|github\.com\/.*\.git/.test(arg)) {
							findings.push({
								...
								fix: {
									description:
										"Use a pinned npm package version instead of a git URL",
									before: `"${arg.substring(0, 40)}"`,
	...
	},
	{
		...
		description:
			"Checks for MCP servers with arguments that disable security features",
		severity: "critical",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
			...
			try {
				...
				const dangerousFlags: ReadonlyArray<{
				...
				}> = [
					{
						pattern: /--no-sandbox/,
						description:
							"Disables sandboxing — process runs with full system access",
					},
					{
						pattern: /--disable-web-security/,
						description:
							"Disables web security policies (CORS, same-origin) — enables cross-site attacks",
					},
					{
						pattern: /--allow-running-insecure-content/,
						description:
							"Allows loading HTTP content over HTTPS — enables MITM attacks",
					},
					{
						pattern: /--unsafe-perm/,
					...
					},
					{
						...
						description:
							"Disables TLS certificate verification — enables MITM attacks",
					},
				];
	...
	},
	{
		...
		description:
			"Checks for MCP servers with both url and command fields, which is ambiguous and potentially dangerous",
		severity: "medium",
		category: "misconfiguration",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
	...
	},
	{
		...
		description:
			"Checks for MCP servers without an explicit env block, which inherit the parent process's full environment including secrets",
		severity: "medium",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
				...
				for (const [name, server] of Object.entries(servers)) {
					...
					if (hasCommand && !hasEnv) {
						findings.push({
							...
							fix: {
								description:
									"Add an explicit env block with only required variables",
								before: `"${name}": { "command": "..." }`,
	...
	},
	{
		...
		description:
			"Checks for MCP servers with database connection strings containing credentials in env or args",
		severity: "high",
		category: "secrets",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
	...
	},
	{
		...
		description:
			"Checks for MCP servers configured to listen on ports below 1024, which require root privileges",
		severity: "medium",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
	...
	},
	{
		...
		description:
			"Checks for MCP servers with CORS set to * in their arguments or environment",
		severity: "medium",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
	...
	},
	{
		...
		description:
			"Checks for MCP servers with credential files (.env, .pem, credentials.json) passed as arguments",
		severity: "high",
		category: "secrets",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
			...
			try {
				...
				const sensitiveFilePatterns: ReadonlyArray<{
				...
				}> = [
					{
						pattern: /\.env\b/,
						description:
							"References .env file — may contain API keys and secrets",
					},
					{
						pattern: /\.pem\b/,
						description:
							"References .pem file — may contain private key material",
					},
					{
						pattern: /credentials\.json/,
						description:
							"References credentials.json — likely contains authentication credentials",
					},
					{
					...
					},
					{
						pattern: /\.p12\b|\.pfx\b/,
						description:
							"References PKCS#12 certificate file — contains private keys",
					},
					{
						...
						for (const { pattern, description } of sensitiveFilePatterns) {
							if (pattern.test(arg)) {
								findings.push({
									...
									fix: {
										description:
											"Use environment variables instead of passing sensitive file paths as arguments",
										before: arg,
	...
	},
	{
		...
		description:
			"Checks for MCP servers configured to listen on 0.0.0.0, exposing the server to the network",
		severity: "high",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
			...
			try {
				...
				for (const [name, server] of Object.entries(servers)) {
					...
					for (const [envKey, envVal] of Object.entries(env)) {
						if (
							/^(?:HOST|BIND|LISTEN)$/i.test(envKey) &&
							envVal === "0.0.0.0"
						) {
							findings.push({
	...
	},
	{
		...
		description:
			"Checks for MCP servers with autoApprove settings that skip user confirmation for tool calls",
		severity: "high",
		category: "mcp",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
						...
						if (key in serverConfig) {
							...
							if (isEnabled) {
								findings.push({
									...
									fix: {
										description:
											"Remove auto-approve to require user confirmation for all tool calls",
	...
	},
	{
		...
		description:
			"Checks for MCP servers without a timeout, which could hang indefinitely or be used for resource exhaustion",
		severity: "low",
		category: "misconfiguration",
		check(file: ConfigFile): ReadonlyArray<Finding> {
			if (file.type !== "mcp-json" && file.type !== "settings-json") return [];
	 ...
	 */
	{
		...
		check(file: ConfigFile): ReadonlyArray<Finding> {
			...
			function isNpxCommand(cmd: string | undefined): boolean {
				...
				return (
					basename === "npx" || basename === "npx.cmd" || basename === "npx.exe"
				);
			}
