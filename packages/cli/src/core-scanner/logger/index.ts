...
/**
 * Structured logger for W.H.Agent scan sessions.
 *
...
export class ScanLogger {
	...
	log(eventType: ScanEventType, data: Record<string, unknown>): void {
		const event: ScanEvent = {
			...
			data: {
				...data,
				_elapsedMs: Math.round(performance.now() - this._startMs),
			},
		};
	...
	writeToFile(
		...
		const content =
			format === "ndjson" ? formatNdjson(log) : JSON.stringify(log, null, 2);
