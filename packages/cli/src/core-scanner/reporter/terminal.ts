...
export function renderTerminalReport(report: SecurityReport): string {
	...
	// Header
	lines.push("");
	lines.push(chalk.bold.cyan("  W.H.Agent Security Report"));
	...
	if (report.findings.length > 0) {
		...
		for (const [severity, findings] of grouped) {
			...
			lines.push(
				`  ${severityIcon(severity)} ${chalk.bold(severity.toUpperCase())} (${findings.length})`,
			);
			lines.push("");
	...
	lines.push(chalk.dim("  W.H.Agent — Security auditor for AI agent configs"));
	lines.push("");
 ...
 */
export function renderInjectionResults(result: InjectionSuiteResult): string {
	const lines: string[] = [];
	...
	lines.push(
		chalk.red(`  ┃  ${"Prompt Injection Testing".padEnd(divider.length - 2)}┃`),
	);
	lines.push(
		chalk.red(
			`  ┃  ${"Active payload testing against config defenses".padEnd(divider.length - 2)}┃`,
		),
	);
	lines.push(chalk.red(`  ┗${divider}┛`));
	lines.push("");
	...
	const blockRate =
		result.totalPayloads > 0
			...
			: 0;
	const blockColor =
		blockRate >= 90 ? chalk.green : blockRate >= 70 ? chalk.yellow : chalk.red;
	...
	lines.push(
		`  Bypassed:       ${result.bypassed > 0 ? chalk.red.bold(`${result.bypassed}`) : chalk.green("0")}`,
	);
	...
	if (result.results.length > 0) {
		...
		lines.push(
			chalk.dim(`    ${"─".repeat(9)} ${"─".repeat(20)} ${"─".repeat(30)}`),
		);
...
export function renderSandboxResults(result: SandboxResult): string {
	...
	lines.push(
		chalk.magenta(
			`  ┃  ${"Sandbox Hook Execution".padEnd(divider.length - 2)}┃`,
		),
	);
	lines.push(
		chalk.magenta(
			`  ┃  ${"Behavioral analysis of hook commands".padEnd(divider.length - 2)}┃`,
		),
	);
	lines.push(chalk.magenta(`  ┗${divider}┛`));
	lines.push("");
	...
	if (result.behaviors.length > 0) {
		...
		for (const behavior of result.behaviors) {
			const exitIcon =
				behavior.exitCode === 0 ? chalk.green("✓") : chalk.red("✗");
			...
			if (behavior.networkAttempts.length > 0) {
				lines.push(
					chalk.yellow(
						`    Network attempts: ${behavior.networkAttempts.length}`,
					),
				);
			...
			if (behavior.fileAccesses.length > 0) {
				lines.push(
					chalk.blue(`    File accesses: ${behavior.fileAccesses.length}`),
				);
				...
				if (behavior.fileAccesses.length > 5) {
					lines.push(
						chalk.dim(`      ... and ${behavior.fileAccesses.length - 5} more`),
					);
				}
			}
...
export function renderTaintResults(result: TaintResult): string {
	...
	lines.push(
		chalk.yellow(
			`  ┃  ${"Taint Analysis — Data Flow Tracking".padEnd(divider.length - 2)}┃`,
		),
	);
	lines.push(
		chalk.yellow(
			`  ┃  ${"Tracking untrusted inputs to dangerous sinks".padEnd(divider.length - 2)}┃`,
		),
	);
	lines.push(chalk.yellow(`  ┗${divider}┛`));
	lines.push("");
 ...
 */
export function renderCorpusResults(result: CorpusValidationResult): string {
	const lines: string[] = [];
	const divider = "━".repeat(56);
	...
	lines.push(
		chalk.cyan(
			`  ┃  ${"Corpus Validation — Scanner Accuracy".padEnd(divider.length - 2)}┃`,
		),
	);
	lines.push(
		chalk.cyan(
			`  ┃  ${"Testing scanner against known attack patterns".padEnd(divider.length - 2)}┃`,
		),
	);
	lines.push(chalk.cyan(`  ┗${divider}┛`));
	lines.push("");
	...
	const rateColor =
		result.detectionRate >= 0.95
			? chalk.green
			: result.detectionRate >= 0.8
				? chalk.yellow
				: chalk.red;
	...
	lines.push(
		`  Missed:          ${result.missed > 0 ? chalk.red(`${result.missed}`) : chalk.green("0")}`,
	);
	...
	if (result.accuracyRecommendations.length > 0) {
		...
		for (const recommendation of result.accuracyRecommendations) {
			...
			if (recommendation.missingRules.length > 0) {
				lines.push(
					chalk.dim(
						`      Missing rules: ${recommendation.missingRules.join(", ")}`,
					),
				);
			}
			if (recommendation.configIds.length > 0) {
				lines.push(
					chalk.dim(`      Configs: ${recommendation.configIds.join(", ")}`),
				);
			}
...
export function renderDeepScanSummary(result: DeepScanResult): string {
	...
	lines.push(
		chalk.bold.cyan(
			`  ║  ${"Deep Scan Summary — All Analysis Layers".padEnd(divider.length - 2)}║`,
		),
	);
	lines.push(chalk.bold.cyan(`  ╚${divider}╝`));
	lines.push("");
	...
	const gradeColor =
		grade === "A" || grade === "B"
			...
			: grade === "C"
				...
				: chalk.red;
	lines.push(
		`  ${chalk.bold("1. Static Analysis")}     ${gradeColor(`Grade: ${grade} (${result.staticAnalysis.score.numericScore}/100)`)}`,
	);
	lines.push(
		chalk.dim(
			`     ${result.staticAnalysis.findings.length} findings from 118 rules`,
		),
	);
	...
	if (result.taintAnalysis) {
		...
		lines.push(
			`  ${chalk.bold("2. Taint Analysis")}      ${flowColor(`${flowCount} tainted flows`)}`,
		);
		lines.push(
			chalk.dim(
				`     ${result.taintAnalysis.sources.length} sources, ${result.taintAnalysis.sinks.length} sinks`,
			),
		);
	} else {
		lines.push(
			`  ${chalk.bold("2. Taint Analysis")}      ${chalk.dim("not available")}`,
		);
	}
	...
	if (result.injectionTests) {
		const blockRate =
			result.injectionTests.totalPayloads > 0
				? Math.round(
						(result.injectionTests.blocked /
							result.injectionTests.totalPayloads) *
							100,
					)
				: 0;
		const blockColor =
			blockRate >= 90
				...
				: blockRate >= 70
					...
					: chalk.red;
		lines.push(
			`  ${chalk.bold("3. Injection Testing")}   ${blockColor(`${blockRate}% blocked`)} (${result.injectionTests.blocked}/${result.injectionTests.totalPayloads})`,
		);
		if (result.injectionTests.bypassed > 0) {
			lines.push(
				chalk.red(
					`     ${result.injectionTests.bypassed} payloads bypassed defenses`,
				),
			);
		}
	} else {
		lines.push(
			`  ${chalk.bold("3. Injection Testing")}   ${chalk.dim("not available")}`,
		);
	}
	...
	if (result.sandboxResults) {
		...
		lines.push(
			`  ${chalk.bold("4. Sandbox Execution")}   ${riskColor(`${riskCount} risks`)} from ${result.sandboxResults.hooksExecuted} hooks`,
		);
	} else {
		lines.push(
			`  ${chalk.bold("4. Sandbox Execution")}   ${chalk.dim("not available")}`,
		);
	}
	...
	if (result.opusAnalysis) {
		const riskColor =
			result.opusAnalysis.auditor.riskLevel === "critical"
				? chalk.red
				: result.opusAnalysis.auditor.riskLevel === "high"
					? chalk.yellow
					: chalk.green;
		lines.push(
			`  ${chalk.bold("5. Opus Pipeline")}       ${riskColor(`Risk: ${result.opusAnalysis.auditor.riskLevel.toUpperCase()}`)} (${result.opusAnalysis.auditor.score}/100)`,
		);
	} else {
		lines.push(
			`  ${chalk.bold("5. Opus Pipeline")}       ${chalk.dim("not available (set ANTHROPIC_API_KEY)")}`,
		);
	}

	lines.push("");
	lines.push(chalk.dim("  ─────────────────────────────────────────"));
	lines.push(chalk.bold.cyan("  W.H.Agent Deep Scan Complete"));
	lines.push("");
...
function formatSkillStatus(
	status: "healthy" | "watch" | "at-risk" | "unobserved",
): string {
	switch (status) {
		case "healthy":
			return "healthy";
