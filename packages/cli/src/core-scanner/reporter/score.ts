...
function computeScore(findings: ReadonlyArray<Finding>): SecurityScore {
	...
	for (const finding of findings) {
		...
		const deduction =
			(SCORE_DEDUCTIONS[finding.severity] ?? 0) * confidenceWeight(finding);
...
function confidenceWeight(finding: Finding): number {
	...
	if (
		finding.runtimeConfidence === "project-local-optional" &&
		finding.category !== "secrets"
	) {
		return 0.75;
	}
	...
	if (
		finding.runtimeConfidence === "plugin-manifest" &&
		finding.category !== "secrets"
	) {
		return 0.5;
	}
