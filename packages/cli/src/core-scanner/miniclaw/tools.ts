 ...
 */
export function createCustomWhitelist(toolNames: ReadonlyArray<string>): {
...
export async function executeToolCall(
	...
	switch (call.tool) {
		...
		case "write":
			return {
				result: await executeWrite(call, sandboxPath, events, sessionId),
				events,
			};
		case "search":
		...
		case "edit":
			return {
				result: await executeEdit(call, sandboxPath, events, sessionId),
				events,
			};
		case "glob":
 ...
 */
export function getToolsByRiskLevel(): Readonly<
	Record<ToolRiskLevel, ReadonlyArray<AllowedTool>>
> {
	return {
		safe: SAFE_TOOLS,
