...
function pickRule(sourceName: string, sinkName: string, isDirect: boolean): string {
    if (CREDENTIAL_SOURCES.has(sourceName) && NETWORK_OUTPUT_SINKS.has(sinkName)) return "TT3";
    if (FILE_READ_SOURCES.has(sourceName) && NETWORK_OUTPUT_SINKS.has(sinkName)) return "TT4";
    if ((NETWORK_INPUT_SOURCES.has(sourceName) || USER_INPUT_SOURCES.has(sourceName)) && EXEC_SINKS.has(sinkName)) return "TT5";
    return isDirect ? "TT1" : "TT2";
}

function getRuleSeverity(ruleId: string): Severity {
    switch (ruleId) {
        case "TT1": return "high";
        case "TT2": return "medium";
        case "TT3": return "critical";
        case "TT4": return "high";
        case "TT5": return "critical";
        default: return "medium";
    }
}

function classify(name: string, categories: { names: Set<string>; label: string }[], def: string): string {
...
export function analyzeTaint(
  ...
  for (const file of files) {
      ...
      function emit(ruleId: string, lineno: number, msg: string, srcName: string, sinkName: string) {
      ...
      function handleAssignment(target: ts.Node, value: ts.Node) {
          ...
          function findSource(n: ts.Node) {
              ...
              if (name && ALL_SOURCES.has(name)) {
                  tainted.set(targetName, { name: targetName, sourceCall: name, lineno });
                  sourceFound = true;
              } else if (name && name.startsWith("process.env")) {
                  tainted.set(targetName, { name: targetName, sourceCall: "process.env", lineno });
                  sourceFound = true;
              } else if (ts.isIdentifier(n)) {
                  // Taint propagation: target = taintedVar
                  const t = tainted.get(n.text);
                  if (t) {
                      tainted.set(targetName, { name: targetName, sourceCall: t.sourceCall, lineno: t.lineno });
                      sourceFound = true;
                  }
              }
      ...
      function checkDirectSources(argNode: ts.Node, sinkName: string, lineno: number) {
          function find(n: ts.Node) {
              ...
              if (name && (ALL_SOURCES.has(name) || name.startsWith("process.env"))) {
                  ...
                  emit(rule, lineno, `Direct flow: ${srcName} (${srcCat}) -> ${sinkName} (${sinkCat})`, srcName, sinkName);
              }
      ...
      function checkTaintedVars(argNode: ts.Node, sinkName: string, lineno: number) {
          function find(n: ts.Node) {
              if (ts.isIdentifier(n)) {
                  ...
                  if (t) {
                      ...
                      const sinkCat = classify(sinkName, SINK_CATEGORIES, "data sink");
                      emit(rule, lineno, `Tainted flow: '${t.name}' from ${t.sourceCall} (line ${t.lineno}, ${srcCat}) -> ${sinkName} (${sinkCat})`, t.sourceCall, sinkName);
                  }
              }
              ts.forEachChild(n, find);
          }
  ...
  // Deduplicate flows
  const uniqueFlows = Array.from(new Map(allFlows.map(f => [f.description, f])).values());
  const severityOrder: Record<Severity, number> = { critical: 0, high: 1, medium: 2, low: 3, info: 4 };
  const sortedFlows = uniqueFlows.sort((a, b) => severityOrder[a.severity] - severityOrder[b.severity]);
  
  // Deduplicate sources and sinks
  const uniqueSources = Array.from(new Map(allSources.map(s => [`${s.file}:${s.line}:${s.label}`, s])).values());
  const uniqueSinks = Array.from(new Map(allSinks.map(s => [`${s.file}:${s.line}:${s.label}`, s])).values());
