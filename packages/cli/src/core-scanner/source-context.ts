...
const CLAUDE_PLUGIN_CACHE_PATH_PATTERN =
	/(^|\/)\.claude\/plugins\/cache(\/|$)/i;
...
export function isPluginCachePath(path: string, scanRoot?: string): boolean {
	...
	if (
		findAllMatches(normalizedPath, CLAUDE_PLUGIN_CACHE_PATH_PATTERN).length > 0
	) {
		return true;
	}
	...
	return (
		findAllMatches(normalizedPath, CLAUDE_SCAN_ROOT_PLUGIN_CACHE_PATH_PATTERN)
			.length > 0
	);
}

function isClaudeScanRoot(scanRoot: string): boolean {
	const normalizedRoot = scanRoot
		.replace(/\\/g, "/")
		.replace(/\/+$/, "")
		.toLowerCase();
