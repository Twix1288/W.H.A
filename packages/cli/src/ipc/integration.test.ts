...
async function waitForSocket(
	...
	throw new Error(
		`Socket file never appeared at ${socketPath} after ${timeoutMs}ms`,
	);
}
...
async function runIntegrationTest() {
	...
	if (useDocker) {
		...
		await execAsync(
			`docker run -d --name ${containerName} --privileged -v ${socketMountDir}:/tmp -v /sys/kernel/debug:/sys/kernel/debug:rw -v /sys/kernel/tracing:/sys/kernel/tracing:rw shield-agent-integration sleep infinity`,
		);
	}
	...
	try {
		if (useDocker) {
			...
			await execAsync(
				`docker exec -d ${containerName} sh -c "/app/shield-agent > /tmp/agent.log 2>&1"`,
			);
		} else {
			console.log(
				"   Waiting for agent (assumed to be started externally via NO_DOCKER)...",
			);
		}
		...
		client.on("network_block", (event) => {
			console.log(
				`   ✅ Received network_block event from eBPF! PID: ${event.pid}, IP: ${event.dst_ip}`,
			);
		...
		if (useDocker) {
			await execAsync(
				`docker exec ${containerName} apt-get update && apt-get install -y curl`,
			);
			await execAsync(
				`docker exec ${containerName} curl -s -m 2 https://1.1.1.1 || true`,
			);
		} else {
