...
import { ShieldEvent } from './events';
            ...
            const tryConnect = () => {
                ...
                this.client.on('error', (err: any) => {
                    ...
                    if (elapsed + currentDelay > maxWaitMs) {
                        this.client?.destroy();
                        reject(new Error(`Failed to connect to shield-agent socket at ${this.socketPath} after ${elapsed}ms: ${err.message}`));
                        return;
                    }
    ...
    private setupStream() {
        ...
        rl.on('line', (line) => {
            ...
            try {
                ...
                if (event.v !== 1) {
                    this.emit('error', new Error(`Unsupported event schema version: ${event.v}`));
                    return;
                }
