...
async function runTests() {
	...
	try {
		...
		try {
			console.log(
				"   Test 1: Connecting to bad socket (testing 5s timeout...)",
			);
		...
		if (!failedAsExpected)
			throw new Error("Test 1 Failed: Expected timeout error");
		console.log(
			"   ✅ Test 1 Passed: Exponential backoff triggered failure after ~5s.",
		);
		...
		server = net.createServer((c) => {
			...
			c.write(
				'{"v":1,"ts":1718000000,"type":"network_block","dst_ip":"198.51.100.1","dst_port":443,"process":"agent.py","pid":1234}\n',
			);
			c.write(
				'{"v":1,"ts":1718000001,"type":"prompt_injection","turn":4,"confidence":0.94}\n',
			);
		...
		goodClient.on("error", (err) => {
			if (err.message.includes("Failed to parse NDJSON"))
				parseErrorReceived = true;
			if (err.message.includes("Unsupported event schema version"))
				versionErrorReceived = true;
		});
		...
		if (!networkBlockReceived)
			throw new Error(
				"Test 2 Failed: Did not receive valid network_block event",
			);
		if (!promptInjectionReceived)
			throw new Error(
				"Test 2 Failed: Did not receive valid prompt_injection event",
			);
		if (!parseErrorReceived)
			throw new Error("Test 2 Failed: Did not handle JSON parse error");
		if (!versionErrorReceived)
			throw new Error("Test 2 Failed: Did not enforce v: 1 schema contract");

		console.log(
			"   ✅ Test 2 Passed: NDJSON parsing, typed emission, and schema enforcement working correctly.",
		);
