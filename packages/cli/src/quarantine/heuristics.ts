...
export class QuarantineHeuristics {
  ...
  async evaluate(meta: PackageMeta): Promise<QuarantineResult> {
    ...
    if (ageInDays < this.MAX_AGE_DAYS && meta.weeklyDownloads < this.MIN_WEEKLY_DOWNLOADS) {
      reasons.push('HIGH RISK: Age + download count combination matches dependency confusion pattern');
    }
