...
- **Linux:** Unprivileged Landlock enforcement and `seccomp-bpf`
- **Windows:** Job Objects and AppContainer (Low Box tokens)
...
Wrap an untrusted agent or script execution inside W.H.Agent's OS-native sandbox:

```bash
wh-agent run ./malicious-agent.js
```
