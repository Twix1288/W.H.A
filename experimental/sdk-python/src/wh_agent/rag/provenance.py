...
def filter_documents_by_risk(
    documents: List[Document], risk_threshold: float = 0.8
) -> List[Document]:
    """Tag documents with provenance/risk, then return only those BELOW the
    threshold.

    This is the enforcement primitive: call it on a retriever's output to drop
    likely prompt-injection documents before they reach the LLM. It is a pure,
    backend-free function — no network, no service. (A LangChain callback cannot
    alter what a retriever returns, so real dropping must happen here, not in a
    callback.)
    """
    processed = process_retrieved_documents(documents)
    return [
        doc
        for doc in processed
        if doc.metadata.get("wh-agent_risk_score", 0.0) < risk_threshold
    ]
